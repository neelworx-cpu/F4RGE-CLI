---
name: 4rged-hooks
description: Use when the user wants to add, write, debug, or configure a 4RGED hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in 4rged.json.
---

# 4RGED Hooks

Hooks are user-defined commands in `4rged.json` that fire at specific points
...
3. Add the entry under `hooks.PreToolUse` in `4rged.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check 4RGED logs.
...
4RGED also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: 4RGED treats `updated_input` as shallow-merge, Claude Code replaces.
