...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (4rged-config).
	activeSkills := []*Skill{
		{Name: "4rged-config", Description: "4rged config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("4rged-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via 4rged://...).
	tracker.MarkLoaded("4rged-config")
	require.True(t, tracker.IsLoaded("4rged-config"))
