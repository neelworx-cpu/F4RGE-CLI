// Package agent is the core orchestration layer for 4rged AI agents.
//
...
import (
	...
	"github.com/neelworx-cpu/F4RGE-CLI/internal/stringext"
...
var userAgent = fmt.Sprintf("4RGED/%s (https://4rged.app/cli)", version.Version)
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (*fantasy.AgentResult, error) {
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "4rged auth" to re-authenticate.`)
			if a.notify != nil {
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("F4RGED_DISABLE_ANTHROPIC_CACHE")); t {
