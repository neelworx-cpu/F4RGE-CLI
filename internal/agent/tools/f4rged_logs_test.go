...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "4rged.log")
...
func TestNewF4rgedLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewF4rgedLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, F4rgedLogsToolName, tool.Info().Name)
}

func TestF4rgedLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 3})
...
func TestF4rgedLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 0})
...
func TestF4rgedLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestF4rgedLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runF4rgedLogs("/nonexistent/path/4rged.log", F4rgedLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestF4rgedLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "4rged.log")
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestF4rgedLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "4rged.log")
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 10})
...
func TestF4rgedLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 1})
...
func TestF4rgedLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestF4rgedLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestF4rgedLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestF4rgedLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "4rged.log")
...
func TestF4rgedLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "4rged.log")
...
func TestF4rgedLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestF4rgedLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestF4rgedLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestF4rgedLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runF4rgedLogs(logFile, F4rgedLogsParams{Lines: 5})
...
func TestF4rgedLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
