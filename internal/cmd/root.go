...
import (
	...
	f4rgedlog "github.com/neelworx-cpu/F4RGE-CLI/internal/log"
	...
	ui "github.com/neelworx-cpu/F4RGE-CLI/internal/ui/model"
...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom 4rged data directory")
	rootCmd.PersistentFlags().BoolP("debug", "d", false, "Debug")
	rootCmd.PersistentFlags().StringVarP(&clientHost, "host", "H", server.DefaultHost(), "Connect to a specific 4rged server host (for advanced users)")
...
4rged run "Guess my 5 favorite Pokémon"

# Run a non-interactively with pipes and redirection
cat README.md | 4rged run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
4rged --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
4rged --yolo

# Run with custom data directory
4rged --data-dir /path/to/custom/.4rged

# Continue a previous session
4rged --session {session-id}
...
// enabled via the F4RGED_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("F4RGED_CLIENT_SERVER"))
	return v
}
...
// F4RGED_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "4rged.log")
	f4rgedlog.Setup(logFile, debug)
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "4rged.log")
		f4rgedlog.Setup(logFile, debug)
	}
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize 4rged server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/4rged/server-<safeHost>/start.lock.
//
...
// Overridable via F4RGED_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("F4RGED_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start 4rged server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach 4rged server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("F4RGED_DISABLE_METRICS")); v {
		return false
	}
...
func createDotF4rgedDir(dir string) error {
