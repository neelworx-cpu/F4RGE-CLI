...
the agent lifecycle, allowing you to both build on top of 4RGED, customize
...
- 4RGED ships with a builtin `4rged-hook` skill write, edit, and configure
  hooks; just tell 4RGED how to configure 4RGED
- 4RGED currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `4rged.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/4rged/4rged.json`), use an absolute path instead.
...
# ($F4RGED_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$F4RGED_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through 4RGED's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), 4RGED falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `4RGED=1`, `AGENT=4rged`, and
  `AI_AGENT=4rged` on top of the `F4RGED_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, 4RGED cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, 4RGED abandons it, logs a
...
Hooks can be added to your `4rged.json` (or `.4rged.json`) at both the global
...
> work fine in project-level `4rged.json` because the project root is also
> your working directory. For **global** config (`~/.config/4rged/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/4rged/4rged.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/4rged/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through 4RGED's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, 4RGED treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as 4RGED will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| `4RGED`                      | Always `1` when running under 4RGED.           |
| `AGENT`                      | Always `4rged`.                                |
| `AI_AGENT`                   | Always `4rged`.                                |
| `F4RGED_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `F4RGED_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `F4RGED_SESSION_ID`           | Current session ID.                            |
| `F4RGED_CWD`                  | Working directory.                             |
| `F4RGED_PROJECT_DIR`          | Project root directory.                        |
| `F4RGED_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `F4RGED_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The `4RGED`, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under 4RGED?" the same way in
either context.
...
Hooks communicate back to 4RGED via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `4rged.json`.
...
If a hook exceeds its timeout, 4RGED cancels its context and treats the
...
if echo "$F4RGED_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells 4RGED to pre-approve the call:
...
case "$F4RGED_TOOL_INPUT_COMMAND" in
...
if [[ "$F4RGED_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $F4RGED_TOOL_NAME\" >> ./tools.log" }
```
...
4RGED hooks are broadly compatible with [Claude Code
...
hooks run under 4RGED unchanged. This document covers the 4RGED-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: 4RGED treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, 4RGED pre-approves the tool call and skips the permission
...
Hooks are part of the 4RGED CLI runtime. File issues in the [F4RGE-CLI repository](https://github.com/neelworx-cpu/F4RGE-CLI).
