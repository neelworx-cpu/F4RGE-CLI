...
name: builtin-skills
description:
  Use when creating a new builtin skill for 4RGED, editing an existing builtin
...
4RGED embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `4rged://skills/` prefix (e.g., `4rged://skills/jq/SKILL.md`).
- The View tool resolves `4rged://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `4rged-config` | `builtin/4rged-config/` | 4RGED configuration help                   |
| `4rged-hooks`  | `builtin/4rged-hooks/`  | Authoring, configuring and debugging hooks |
